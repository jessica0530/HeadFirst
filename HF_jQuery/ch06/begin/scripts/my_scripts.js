$(document).ready(function(){
	...
	var hand = {
		...
		sumCardTotal: function(){
			...
			if(this.current_total > 21){
				...
				$("#imgResult").attr('src','../end/images/x2.png');
				$("#hdrResult").html("BUST!")
							   .attr('class', 'lose');
			}else if(this.current_total == 21){
				$("#btnStick").trigger("click");
				$("#imgResult").attr('src','../end/images/check.png');
	...
	function getRandom(num) {
		return my_num = Math.floor(Math.random()*num);
	}
	...
	function hit() {
		...
		do {
			...
			if(!$.inArray(index,used_cards) > -1) {
				...
				$("<img>").appendTo($d).attr('src','../end/images/cards/' + c.suit + '/' + c.name + '.jpg')
